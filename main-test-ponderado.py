...
def encontrar_vecinos_mas_cercanos_ponderado(instancias, instancia_nueva, k):
    distancias = []
    for instancia in instancias:
        distancia = calcular_distancia(instancia, instancia_nueva)
        distancias.append((instancia, distancia))
    ...
    vecinos = []
    for i in range(k):
        vecino = distancias[i][0]
        peso = 1 / (distancias[i][1] + 1e-6)  # Evitar división por cero
        vecino.peso = peso
...
for i, instancia_nueva in enumerate(instancias_random, start=1):
    ...
    # Modificación en el ejemplo de uso para utilizar k-NN ponderado
    for k in range(1, 7):
        # Encuentra los vecinos más cercanos con k-NN ponderado
        vecinos_mas_cercanos = encontrar_vecinos_mas_cercanos_ponderado(instancias, instancia_nueva, k)

        # Calcula la distancia ponderada para cada vecino y aplica ponderación
        for vecino in vecinos_mas_cercanos:
            vecino.distancia = calcular_distancia(vecino, instancia_nueva) * vecino.peso
