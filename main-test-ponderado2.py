...
# Función para realizar la clasificación ponderada de una instancia
def clasificar_ponderado(vecinos, instancia_nueva):
    total_clases = {0: 0, 1: 0}
    total_pesos = 0
    
    for vecino in vecinos:
        # Calcular la ponderación inversa de la distancia
        peso = 1 / (vecino.distancia + 1e-10)
        total_pesos += peso
        total_clases[vecino.clase] += peso
    
    # Normalizar las ponderaciones
    if total_pesos > 0:
        for clase in total_clases:
            total_clases[clase] /= total_pesos
    
    # Determinar la clase final
    clase_final = max(total_clases, key=total_clases.get)
    return clase_final
...
for i, instancia_nueva in enumerate(instancias_random, start=1):
    ...
    for k in range(1, 7):
        ...
        clase_predicha = clasificar_ponderado(vecinos_mas_cercanos, instancia_nueva)
        print(f"Clase predicha para k={k}: {clase_predicha}")
